...
### `GET /api/stocks?tickers=AAPL,NVDA`

Returns live quotes for requested tickers.

```json
{
  "quotes": [
    { "ticker": "AAPL", "price": 189.84, "change": 2.34, "changePercent": 1.25, ... }
  ],
  "updatedAt": "2024-06-15T10:30:00Z"
}
```

### `GET /api/stocks/:ticker?tf=1M`
...
```json
{
  "ticker": "NVDA",
  "quote": { ... },
  "candles": [ { "time": "...", "open": 860, "high": 875, "low": 855, "close": 870, "volume": 41800000 } ],
  "indicators": { "rsi": 62.4, "macd": 3.12, "ema20": 865.20, ... },
  "sentiment": {
    "score": 0.88,
    "bias": "BULLISH",
    "keyRisk": "Export restrictions limit revenue ceiling",
    "catalyst": "Blackwell GPU backlog exceeds 12-month supply",
    "confidence": 91
  },
  "signals": [
    { "type": "LONG", "entry": 870, "stopLoss": 844, "target1": 922, "target2": 974, "confidence": 88, "riskReward": 2.0 }
  ],
  "news": [ ... ]
}
```
...
Sentiment only, cached for 30 minutes.

### `GET /api/watchlist` (auth required)

Returns user's watchlist with live prices.
